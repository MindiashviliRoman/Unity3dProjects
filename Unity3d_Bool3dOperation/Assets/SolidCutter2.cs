...
//                t1[i].CreateControCrossController(t2.Count);
                kk++;
                for (int j = 0; j < t2.Count; j++) {
/*
                    if (i == 0) {
                        t2[j].CreateControCrossController(t1.Count);
                    }
*/
                        ...
                        */
                        ...
                        for (int ii = 0; ii < BordCount; ii++) {
                            if (PerTriPnts[ii].Count > 0) {
                                ...
                                List<int> tris = Triangulation.GetTriangles(tmpVectors, 0, t1[i].Norm);
                                t1[i].IsCrossed = true;
                                int indx = t1[i].IndSubMsh;
                                newBody.AddNewPoligonAttributes(indx, tris, PerTriPnts[ii]);
                            }
                        }
                        int HoleCount = -1;
                        ...
                        for (int ii = 0; ii < HoleCount; ii++) {
                            if (PerTriPntsHoles[ii].Count > 0) {
                                ...
                                t1[i].IsCrossed = true;
                                int indx = t1[i].IndSubMsh;
                                newBody.AddNewPoligonAttributes(indx, tris, PerTriPntsHoles[ii]);
                            }
                        }
			...
			if (flgIsCrossedBody) {
	            ...
	            for (int i = 0; i < pols.Length; i++) {
	                if (pols[i].Pnts.Count > 2) {
                        ...
                        newBody.AddNewPoligonAttributes(indx, tris, pols[i].Pnts);
	                }
	            }
	            ...
	            for (int i = 0; i < t2.Count; i++) {
                    if (t2[i].NetKeys.Count > 0) {
                        ...
                        for(int j = 0; j< t2[i].Pnts.Count; j++) {
                            ListGO.Add(VectorExtension.CreatePntGO(t2[i].Pnts[j].V.ToString(), t2[i].Pnts[j].V));
                        }
                        */
                        //SortSegs неверно отрабатывает для InnerHoles. Добавляет один лишний конечный сегмент!!!!!!!!!!!!!!
                        ...
                        if (SegsOfIntsct.InnerHoles.Count > 0) {
                            ...
                            for (int j = 0; j < PerTriPntsHoles.Length; j++) {
                                for (int z = 0; z < PerTriPntsHoles[j].Count; z++) {
                                    ListGO.Add(VectorExtension.CreatePntGO("Body2"+j.ToString()+"_"+z.ToString(), PerTriPntsHoles[j][z].V));
                                }
                            }
                            */
                        }
                        ...
                        for (int ii = 0; ii < BordCount; ii++) {
                            if (PerTriPnts[ii].Count > 0) {
                                ...
                                List<int> tris = Triangulation.GetTriangles(tmpVectors, 0, t2[i].Norm);
                                t2[i].IsCrossed = true;
...
//            private int side=-1;
            ...
            public bool flgIsTriVertex = false;
            ...
            public bool IsCrossBorderedPnt {
                set { isCrossBorderedPnt = value; }
