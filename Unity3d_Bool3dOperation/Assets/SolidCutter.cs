#define TestPnts1
#define AllPntsAndPoligons

#undef TestPnts1
#undef AllPntsAndPoligons
...
namespace Solid{
    public class SolidCutter: MonoBehaviour{
        ...
        public static Mesh DeductValue(Mesh m3_new, GameObject Basic, GameObject go, ref Material[] mats, ref PntsAttributes connectVerts, int mode = 0) 
        {
            ...
            for (int i = 0; i< 2; i++) 
            {
                for(int j = 0; j< bodyes[i].Borders.Count; j++) 
                {
                    ListGO.Add(ActionLines.DrawLine(bodyes[i].Borders[j].Pnts[0], bodyes[i].Borders[j].Pnts[1], Color.cyan, "Body_" + i.ToString() + "_Border_" + j.ToString()));
                }
                for(int j = 0; j< bodyes[i].Poligons.Length; j++) 
                {
                    Vector3 p = new Vector3();
                    for(int z = 0; z< bodyes[i].Poligons[j].Pnts.Count; z++) 
                    {
                        p += bodyes[i].Poligons[j].Pnts[z].V;
                    }
                    p = p / bodyes[i].Poligons[j].Pnts.Count;
                    ActionLines.Text3D(p, Color.red, "body_"+i.ToString()+ "_Polygon_" + j.ToString());
                }
            }
#endif
...
//					connectVerts.externalPnts = listLinks.GetExternIndxs ();
 ...
 /*
                ...
                for (int i = 0; i < SegsOfIntsct.BorderHoles.Lst.Count; i++) {
                    for (int j = 0; j < SegsOfIntsct.BorderHoles.Lst[i].Hole.Count; j++) {
                        ListGO.Add(VectorExtension.CreatePntGO("Border_" + i.ToString() + "_" + j.ToString() + "_" + SegsOfIntsct.BorderHoles.Lst[i].Hole[j].IsTriVertex + "_"+ SegsOfIntsct.BorderHoles.Lst[i].Hole[j].IsCrossBorderedPnt + "_" + SegsOfIntsct.BorderHoles.Lst[i].Hole[j].IsBorderedPnt + "_" + SegsOfIntsct.BorderHoles.Lst[i].Hole[j].IsTriVertex  + "_" + SegsOfIntsct.BorderHoles.Lst[i].Hole[j].V, SegsOfIntsct.BorderHoles.Lst[i].Hole[j].V));
                    }
                }
                ...
                for (int i = 0; i < SegsOfIntsct.InnerHoles.Lst.Count; i++) {
                    for(int j=0; j< SegsOfIntsct.InnerHoles.Lst[i].Hole.Count; j++) {
                        ListGO.Add(VectorExtension.CreatePntGO("Inner_" + i.ToString() + "_" + j.ToString() + "_" + SegsOfIntsct.InnerHoles.Lst[i].Hole[j].IsTriVertex + "_" + SegsOfIntsct.InnerHoles.Lst[i].Hole[j].IsCrossBorderedPnt + "_" + SegsOfIntsct.InnerHoles.Lst[i].Hole[j].IsBorderedPnt + "_" + SegsOfIntsct.InnerHoles.Lst[i].Hole[j].IsTriVertex + "_" + SegsOfIntsct.InnerHoles.Lst[i].Hole[j].V, SegsOfIntsct.InnerHoles.Lst[i].Hole[j].V));
                    }
                }
 #endif
 */     
                ...
                if (flgExistsInnerHoles) 
                {
                    if (flgExistsBorderHoles) 
                    {
                        for (int i = 0; i < PerTriPnts.Count; i++) 
                        { //необходимо не добавлять в конце точку GetPerimetrPnts
                ...
                if (flgExistsInnerHoles)
                {
                    for (int ii = 0; ii < PerTriPntsHoles.Count; ii++)
                    {
                        if (PerTriPntsHoles[ii].Count > 0) 
                        {
                            ...
                            List<int> tris = Triangulation.GeneralTriangulation(tmpVectors2, 0, pol.PrjNorm, true, flgSide);
                            pol.IsCrossed = true;
                            int indx = pol.IndSubMsh + offsetSubMshNum;
        ...
        public static Holes SortSegs(List<Point> Keys, Poligon pol) 
        {
            ...
            bool flgIsAllBordered = false;
            do 
            {
                flgIsAllBordered = false;
                ...
                flgIsAllBordered = nxtPnt.IsCrossBorderedPnt;
                ...
                if (bonds != null) 
                {
                        ...
                        do 
                        {
                            for (int i = 0; i < nxtbonds.Count; i++) 
                            {
                                if (nxtbonds[i] != null) 
                                {
                                    ...
                                    flgIsAllBordered = flgIsAllBordered && CurPnt[i].IsCrossBorderedPnt;
                    ...
                    if (!(flgIsAllBordered && Seq.Count > 2)) 
                    {
                        if (flgBordered) 
                        {
                            pol.bHoles.BorderHoles.Lst.Add(new HoleParam(Seq));
                            pol.bHoles.BorderHoles.NormCutts.Add(SeqNrmCutter);
                            pol.bHoles.BorderHoles.CreateLinkToNormCutt();
                        }
                        else 
                        {
                            pol.bHoles.InnerHoles.Lst.Add(new HoleParam(Seq));
        ...
        public static List<List<Point>> GetPerimetrPnts(Poligon pol, HoleAttribute holeAttr, bool flg, int indBody)
        {
            ...
            //Цикл по всем точкам данного треугольника
            for (int i = 0; i < polPntsCnt; i++)
            {
                //Идем циклом по всем граничным "срезам" (отдельным линиям пересечения, пересекающим границу дан. треугольника)
                for (int k = holeAttr.Lst.Count - 1; k >= 0; k--) 
                {  
                    int cntPnts = (holeAttr.Lst[k].Hole.Count - 1);//Индекс последней точки в текущем срезе
                    int ck = 0;
                    ...
                    for (int j = 0; j < 2; j++) 
                    {
                        //Берем начальную и конечную точки граничного "среза"
                        Point curBrdPnt = holeAttr.Lst[k].Hole[j * cntPnts];
                        for (int z = 0; z < curBrdPnt.Side.Count; z++) 
                        {
                            //Если точка "среза" на текущей стороне треугольника
                            if (i == curBrdPnt.Side[z])
                            {
                                ck++;
            ...
            while (LSlicesAllCnt > 0 && cnt > -1 && kc < 1000) 
            {
                ...
                if ((tmpflgSign * VectorExtension.SignProjectV3toV3(v2 - v1, curSlice.GetStartPositionNrmCutt())) < 0)
                {
                    //Проблема в CutterNorm    
                    fp = curBordPnt;
                    if (indxFstSide == -1) 
                    {
        ...
        public static Slice NearPoint(Vector3[] vTris, List<Point> PntsOnCurSide, HoleAttribute BordHole, int k, int curSideTri, Vector3 nrmSec, int flgSign)
        {
            ...
            if (LDistance.Count == 2)
            {
                ...
                if (PntsOnCurSide[0].IsCrossBorderedPnt) {
                    if(IsNoGoodWise(BordHole.Lst[k].Hole, vTris[curSideTri + 1] - vTris[curSideTri])) 
                    { 
                        flgRev = true;
            ...
            //Создаём направление
            List<Point> curBordHolePnts = BordHole.Lst[k].Hole;
            ...
            //Если на текущей стороне треугольника только одна точка
            if (PntsOnCurSide.Count == 1)
            {
                Point tmpSecPnt = curBordHolePnts[secondPnt];//another end pnt (border pnt)
                //Цикл по всем точкам данного треугольника
                for (int i = 0; i < vTris.Length - 1; i++) 
                {   
                    for (int z = 0; z < tmpSecPnt.Side.Count; z++) 
                    {
                        //Если точка "среза" на текущей стороне треугольника
                        if (i == tmpSecPnt.Side[z]) 
                        {
...
//			float f = 100f;//Увеличиваю точность при расчётах (с учётом моих округлений в будущем)
        ...
        //Not absolute for compound mesh!
        public static List<List<Point>> GetArrWithHole(HoleAttribute holeAttr, Poligon pol, bool flg, int flgMode, int indBody)
        {
            ...
            List<HoleParam> holes = holeAttr.Lst;
            ...
            List<SubPerimWrap> SubPerimetersFromHoles = new List<SubPerimWrap>();
            List<List<Vector3[]>> normCutts = holeAttr.NormCutts;
            List<int> holesChecker = new List<int>();
            holesChecker.Capacity = holesCnt;//max
            ...
            for (int i = holesCnt-1; i >-1; i--) 
            {
                HoleParam curHolePar = holes[i];
                List<Point> curHole = curHolePar.Hole;
                ...
                if ((double)Vector3.Dot(Vector3.Cross(v2, v1), pol.PrjNorm) > 0) 
                {
                    Vector3 vSecat = curCutts[left][1];
                    curHolePar.IsDirect = true;
                    curHolePar.IsSubHole = flgInt * (Vector3.Dot(v_2, vSecat)) < 0;
                } 
                else 
                {
                    Vector3 vSecat = curCutts[left][1];
                    curHolePar.IsDirect = false;
                    curHolePar.IsSubHole = flgInt * (Vector3.Dot(v_2, vSecat)) > 0;
                }

                if (curHolePar.IsSubHole) 
                {
                    //Create good wise:
                    if (!curHolePar.IsDirect)
                    {
                        curHolePar.Reverse();
                    }
                    SubPerimetersFromHoles.Add(new SubPerimWrap(curHolePar.Hole));// it is hole inside another hole
                } 
                else 
                {
                    holesChecker.Add(i);
            ...
            //start with SubPerimetersFromHoles
            List<SubHoleWrap> outSubPerimForHoles = new List<SubHoleWrap>();
            List<List<Point>> WithoutHolesSubPerim = new List<List<Point>>();
            int startCntHole = holesChecker.Count;
            bool curFlg = false;
            bool flgInitHoleCnt = true;
            for (int ii =  0; ii < holesChecker.Count; ii++)
            {
                int curIndxHole = holesChecker[ii];
                SubHoleWrap curHoleWrap = null;
                for (int i = 0; i < SubPerimetersFromHoles.Count; i++) 
                {
                    float curMinDistForSubPerToHol = float.MaxValue;
                    curFlg = false;
                    cntCurRayCross = 0;
                    if (flgInitHoleCnt)
                    {
                        SubPerimetersFromHoles[i].cntHoles = startCntHole;
                    }
                    List<Point> curShape = SubPerimetersFromHoles[i].cSubPerim;
                    for (int j = 0; j < curShape.Count - 1; j++) 
                    {
                        float curDist = float.MaxValue;
                        //enouth to chech one of pnt of hole
                        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! возможно нужно учесть flg
                        curFlg = IsInnerPntCurShapeWithDist(holes[curIndxHole].Hole[0].ProjV, curShape[j].ProjV, curShape[j + 1].ProjV, ref curDist);
                        if (curFlg) 
                        {
                            cntCurRayCross++;
                            if(curDist < curMinDistForSubPerToHol) 
                            {
                                curMinDistForSubPerToHol = curDist;
                            }
                        }
                    }
                    if (cntCurRayCross % 2 > 0)
                    {
                        if (curHoleWrap == null) 
                        {
                            curHoleWrap = new SubHoleWrap(ii);
                        }
                        curHoleWrap.Add(i, curMinDistForSubPerToHol);
                    }
                    else 
                    {
                        SubPerimetersFromHoles[i].cntHoles--;
                        if(SubPerimetersFromHoles[i].cntHoles == 0) 
                        {
                            Result.Add(SubPerimetersFromHoles[i].cSubPerim);
                        }
                    }
                ...
                if (flgInitHoleCnt) 
                {
                    flgInitHoleCnt = false;
                }
                if (curHoleWrap != null) 
                {
                    outSubPerimForHoles.Add(curHoleWrap);
                }
            }
            int offst = 0;
            if (startCntHole == 0) 
            ...
            {
                for (int i = outSubPerimForHoles.Count-1; i > -1; i--) 
                {
                    int checkerInds = outSubPerimForHoles[i].Index;
                    SubPerimWrap curSubPerimWrap = SubPerimetersFromHoles[outSubPerimForHoles[i].NearestSubPerimIndx];
                    if (curSubPerimWrap.IsAdded)
                    {
                        indexBelongsHole[curSubPerimWrap.tIndx].Add (holesChecker[checkerInds]);
                        holesChecker.RemoveAt(checkerInds);
                    }
                    else 
                    {
                        SubPerimeters.Insert(0, curSubPerimWrap.cSubPerim);
                        curSubPerimWrap.tIndx = indexBelongsHole.Count;
                        curSubPerimWrap.IsAdded = true;
                        indexBelongsHole.Insert(0, new List<int>(new int[] { holesChecker[checkerInds] }));
                        holesChecker.RemoveAt(checkerInds);
                        offst++;
                    }
                }
            ...
            for (int i = offst; i < SubPerimeters.Count; i++)
            {
                ...
                if (curShape.Count > 0) 
                {
                    ...
                    if (Vector3.Dot(Vector3.Cross(v2, v1), pol.PrjNorm) < 0) 
                    {
                        //Create goodwise for outShapes
                        curShape.Reverse();
                    }
                    ...
                    for (int ii = holesChecker.Count - 1; ii > -1; ii--) 
                    {
                        ...
                        for (int j = 0; j < curShape.Count - 1; j++)
                        {
                            curFlg = IsInnerPntCurShape(holes[holesChecker[ii]].Hole[0].ProjV, curShape[j].ProjV, curShape[j + 1].ProjV);
                            if (curFlg)
                            {
                                cntCurRayCross++;
                            }
                        ...
                        if (cntCurRayCross % 2 > 0) 
                        {
                            curHolesInds.Add(holesChecker[ii]);
                            holesChecker.RemoveAt(ii);
                        }
                    }
                    if (curHolesInds.Count > 0) 
                    {
            ...
            {
                kCntHole++;
                SubPerimeters = GetSubPerimetersByHole(SubPerimeters, holes, pol, kCntHole, ref indexBelongsHole, ref Result);
            } 
            while (indexBelongsHole.Count > 0 && kCntHole < 100000);//temporary
        ...
        public static List<List<Point>> GetSubPerimetersByHole(List<List<Point>> Shapes, List<HoleParam> HolesPnts,
            Poligon pol,
            int cntCutHole, //List<int> shapeLeftIndx,
                ...
                for (int i = indsHoles.Count - 1; i > -1; i--) 
                {
                    ...
                    if (cnt > 0)
                    {
                        ...
                        for (int j = cnt - 1; j > -1; j--)
                        {
                            ...
                            HoleParam curHolePar = HolesPnts[curIndsHole];
                            List<Point> curHole = curHolePar.Hole;
                            if (!curHolePar.IsSubHole) 
                            {
                                //Pnts of curHole
                                for (int z = 0; z < curHole.Count - 1; z++)
                                {
                                    ...
                                    if (flgCatchOneHole)
                                    {
                                        ...
                                        for (int zz = j - 1; zz > -1; zz--) 
                                        {
                                            ...
                                            List<Point> otherHole = HolesPnts[otherIndsHole].Hole;
                                        ...
                                        bool flgDirect = curHolePar.IsDirect;
        ...
        private static bool IsInnerPntCurShapeWithDist(Vector2 holePnt, Vector2 curShapePnt, Vector2 nxtShapePnt, ref float r) 
        {
            //horisontal ray
            ...
            if (delta2 > VectorExtension.rFactor) 
            {
                ...
                {
                    flg = VectorExtension.IsCrossHorizontalRayWithDist(curShapePnt - holePnt, nxtShapePnt - holePnt, ref r);
                }
            }
            return flg;
        }
...
#region BodyDefinitionClass
        ...
        public class BodyCreator
        {
            ...
            private void MergeMsh(int subMsh, ref int trisOffst, ref int vertOffst, 
                ref BodyRawAttributes ba, ref LinksCreatorPar links, ref List<int> newLink)
            {
...
#region PoligonDefinitionClass
        public class Poligon: System.IEquatable<Poligon>
        {
			...
			public Poligon(List<Point> points, int[] setBorders, int[] nbrs, int iSubMsh, int direct = 1) //, bool forvardDirect = true)
            {
				...
				indSubMsh = iSubMsh;
                normSecator = Vector3.Cross(points[2].V - points[1].V, points[0].V - points[1].V);//Vector3.Cross(points[2].V - points[0].V, points[1].V - points[0].V);
                isSliser = direct < 0;
                if (isSliser)
                {
                    norm = -normSecator;
            ...
            public bool CrossToPoligon(Poligon other, Body[] bds, int indexThis, int indexOther)
            {
                ...
                */
                ...
                bool flgIsIncludedBorder = false;
                bool flgIsUnParallel = false;
                float t = 0f;
                float Delta = 0f;

                bool flgPntOnlyOnBorder = false; //if pnt is crossPnt of two border on border of triangle

                int otherAnothPolyInt = -1;
                ...
                for (int i = 0; i < tt1.Length - 1; i++)
                {
                    ...
                    flgIsIncludedBorder = false;
                    flgIsUnParallel = false;
                    ...
                    {
                        flgIsIncludedBorder = IsIncludedBorder(tt2, curBorder.Pnts, ref Delta, ref flgIsUnParallel, ref t);
                        WrapVectBox curWrapVectBox = new WrapVectBox(cVect, flgIsIncludedBorder);
                        if (flgIsUnParallel) 
                        {
                            if (CrossTrianglBySegment(ref cP, tt2, curBorder.Pnts, Delta, t,  ref flgCrossBordering, ref side, ref flgIsTriVertex, ref NumVert))
                            {
                                ...
                                if (flgCrossBordering) 
                                {
                                    ...
                                    otherAnothPolyInt = anotherBord.IndxNeighbors[1 - other.neighborsIndx[side]];
                                    bool flgPersist = false;

                                    //OtherAnotherPolygon
                                    if (curBorder.CrossPntWithPlane[otherAnothPolyInt] != null)
                                    {
                                        //if not flgIsIncluded
                                        if (!curBorder.CrossPntWithPlane[otherAnothPolyInt].FlgIsIncludedToPoligon)
                                        {
                                            flgPersist = true;
                                        }
                                    }
                                    ...
                                    {
                                        List<Point> tt_Pnts = bds[1].Poligons[otherAnothPolyInt].pnts;
                                        Vector3[] tt_ = new Vector3[tt_Pnts.Count + 1];
                                        for (int iz = 0; iz < tt_Pnts.Count; iz++)
                                        {
                                            tt_[iz] = tt_Pnts[iz].V;
                                        }
                                        tt_[tt_Pnts.Count] = tt_[0];

                                        if (!IsIncludedBorder(tt_, curBorder.Pnts, ref Delta, ref flgIsUnParallel, ref t)) 
                                        {
                                            flgPersist = true;
                                        }
                                    }
                                    ...
                                    //AnotherBord
//                                    WrapVectBox curCopyWrapVectBox = curWrapVectBox.Copy();
//                                    bordrLink = anotherBord;
                                    flgPntOnlyOnBorder = false;
                                    ...
                                    {
                                        List<Point> tt_Pnts = bds[0].Poligons[indexThis].pnts;
                                        Vector3[] tt_ = new Vector3[tt_Pnts.Count + 1];
                                        for (int iz = 0; iz < tt_Pnts.Count; iz++)
                                        {
                                            tt_[iz] = tt_Pnts[iz].V;
                                        ...
                                        if (!IsIncludedBorder(tt_, anotherBord.Pnts, ref Delta, ref flgIsUnParallel, ref t))
                                        {
                                            anotherBord.CrossPntWithPlane[indexThis] = curWrapVectBox;// curCopyWrapVectBox;//curWrapVectBox;//new WrapVectBox(cVect);
//                                            LinkToCurAnothBrds[0] = indexThis;
                                        }
                                        else
                                        {
                                            flgPntOnlyOnBorder = true;
                                        }

                                    }
                                    else 
                                    {
                                        if (!anotherBord.CrossPntWithPlane[indexThis].FlgIsIncludedToPoligon)
                                        {
                                            ...
                                            if (savedVB != null)
                                            {
                                                ...
                                                if (!flgChng && anotherBordCrssPnt != null)
                                                {
                                    ...
                                    if (anotherBord.CrossPntWithPlane[thisAnothPolyInt] == null)
                                    {
                                        ...
                                        if (!IsIncludedBorder(tt_, anotherBord.Pnts, ref Delta, ref flgIsUnParallel, ref t))
                                        {
                                            anotherBord.CrossPntWithPlane[thisAnothPolyInt] = curWrapVectBox;//curCopyWrapVectBox;// curWrapVectBox;//new WrapVectBox(cVect);
//                                            LinkToCurAnothBrds[1] = thisAnothPolyInt;
                                            flgPntOnlyOnBorder = false;
                                        } 
                                        else
                                        {
                                            flgPntOnlyOnBorder = flgPntOnlyOnBorder && true;
//                                            curWrapVectBox.Unconsidered = flgPntOnlyOnBorder;
                                        }
                                    }
                                    else
                                    {
                                        flgPntOnlyOnBorder = false;
//                                        flgPntOnlyOnBorder = anotherBord.CrossPntWithPlane[thisAnothPolyInt].flgIsIncludedToPoligon;
                                    }
                                }
                            }
                        }
                        ...
                        if (flgPntOnlyOnBorder)
                        {
//                            cVect = null;
//                            curBorder.CrossPntWithPlane[indexOther] = null;
//                            curBorder.CrossPntWithPlane[otherAnothPolyInt] = null;
                            curBorder.CrossPntWithPlane[indexOther].Unconsidered = flgPntOnlyOnBorder;
                            curBorder.CrossPntWithPlane[otherAnothPolyInt].Unconsidered = flgPntOnlyOnBorder;
                        }
                    }
                    else 
                    {
                    ...
                    if (cVect != null && !curBorder.CrossPntWithPlane[indexOther].Unconsidered)// !flgPntOnlyOnBorder) {// !curBorder.CrossPntWithPlane[indexOther].Unconsidered) {
                    {
...
//                            p.SetEqualAverage(cPcL);
                            ...
                            //If was be added from t2 cicle
                            if (cVect.flgCrossBordering && !cPcL.IsCrossBorderedPnt)
                            {
                        ...
                        //if bordering then it will be added in cicle by t2
...
//                                p.SetEqualAverage(cPcL);
                ...
                for (int i = 0; i < tt2.Length - 1; i++)
                {
                    ...
                    {
                        flgIsIncludedBorder = IsIncludedBorder(tt1, curBorder.Pnts, ref Delta, ref flgIsUnParallel, ref t);
                        WrapVectBox curWrapVectBox = new WrapVectBox(cVect, flgIsIncludedBorder);
                        if (flgIsUnParallel)
                        {
                            if (CrossTrianglBySegment(ref cP, tt1, curBorder.Pnts, Delta, t, ref flgCrossBordering, ref side, ref flgIsTriVertex, ref NumVert)) 
                            {
                                cVect = new VectBox(flgCrossBordering, flgIsTriVertex, side, NumVert, ref cP);
                                curWrapVectBox.VB = cVect;
                                if (flgCrossBordering)
                                {
                                    ...
                                    //Save into neighbor poligon
                                    otherAnothPolyInt = anotherBord.IndxNeighbors[1 - this.neighborsIndx[side]];
                                    bool flgPersist = false;
                                    ...
                                    {
                                        List<Point> tt_Pnts = bds[0].Poligons[otherAnothPolyInt].pnts;
                                        Vector3[] tt_ = new Vector3[tt_Pnts.Count + 1];
                                        for(int iz = 0; iz< tt_Pnts.Count; iz++) {
                                            tt_[iz] = tt_Pnts[iz].V;
                                        }
...
//                                    WrapVectBox curCopyWrapVectBox = curWrapVectBox.Copy();
                                    ...
                                    {
                                        List<Point> tt_Pnts = bds[1].Poligons[indexOther].pnts;
                                        Vector3[] tt_ = new Vector3[tt_Pnts.Count + 1];
                                        for (int iz = 0; iz < tt_Pnts.Count; iz++)
                                        {
                                            tt_[iz] = tt_Pnts[iz].V;
                                        ...
                                        if (!IsIncludedBorder(tt_, anotherBord.Pnts, ref Delta, ref flgIsUnParallel, ref t))
                                        {
                                            anotherBord.CrossPntWithPlane[indexOther] = curWrapVectBox;// curCopyWrapVectBox;//curWrapVectBox;//new WrapVectBox(cVect);
//                                            LinkToCurAnothBrds[0] = indexOther;
                                        }
                                        else 
                                        {
                                            flgPntOnlyOnBorder = true;
                                    ...
                                    {
                                        if (!anotherBord.CrossPntWithPlane[indexOther].FlgIsIncludedToPoligon)
                                        {
                                            ...
                                            if (savedVB != null) {
                                                ...
                                                if (!flgChng && anotherBordCrssPnt != null) 
                                                {
                                                    // почему anotherBordCrssPnt != null!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                                                    cVect.V = anotherBordCrssPnt.V;//Save general coordinate of pnt
                                                }
                                            }
                                        } 
...
//                                        flgPntOnlyOnBorder = anotherBord.CrossPntWithPlane[thisAnothPolyInt].flgIsIncludedToPoligon;
                        ...
                        {
//                            cVect = null;
//                            curBorder.CrossPntWithPlane[indexThis] = null;
//                            curBorder.CrossPntWithPlane[otherAnothPolyInt] = null;
                            curBorder.CrossPntWithPlane[indexThis].Unconsidered = flgPntOnlyOnBorder;
                            curBorder.CrossPntWithPlane[otherAnothPolyInt].Unconsidered = flgPntOnlyOnBorder;
                        }
                    }
                    else 
                    ...
                    if (cVect != null && !curBorder.CrossPntWithPlane[indexThis].Unconsidered)  //!flgPntOnlyOnBorder) {// !curBorder.CrossPntWithPlane[indexThis].Unconsidered) {
                    {
...
//                            p.SetEqualAverage(cPcL);
                    ...
                    //if (cVect != null && cVect.V == new Vector3(19.05f, 30f, 5.5f)) {
                    //    string ssss = "";
                    //}
                }
                ...
                {
                    //consider only 2 crss pnts between two trianles
            ...
            public bool IsIncludedBorder(Vector3[] tri, Vector3[] seg, ref float Delta, ref bool flgIsUnParallel, ref float t)
            {
                Vector3 A = tri[0];
                Vector3 B = tri[1];
                Vector3 C = tri[2];
                ...
                Vector3 crssP3P4 = Vector3.Cross(p3, p4);//normal for plane of triangle //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Dublicated!!!!!!!!!!!!!!!!!!!!!!!                   |

                Delta = Vector3.Dot(p2, crssP3P4);

                float l = -0.00001f;
                float almostZero = 0.00001f;
                bool flgIncluded = false;
                flgIsUnParallel = false;
...
/*
            public WrapVectBox[] CrossPntWithPlane {
                set { crossPntWithPlane = value; }
                get { return crossPntWithPlane; }
            }
*/
...
#region PointDefinitionClass
        ...
        {
            public VectBox VB;
            public bool FlgIsIncludedToPoligon;//The border inserted in plane of poligon
            public bool Unconsidered;//For consider this pnt or not.

            public WrapVectBox(VectBox nvb, bool flgIsIncluded)//, bool calculated)
            {
                this.VB = nvb;
                this.FlgIsIncludedToPoligon = flgIsIncluded;
                this.Unconsidered = false;
            }

            public WrapVectBox Copy() 
            {
                return new WrapVectBox(this.VB, this.FlgIsIncludedToPoligon);//, this.IsCalculated);
            }
        }
        public class VectBox 
        {
            ...
            public VectBox(bool flgCrBord, bool flgIsTriV, int sd, int nVert, ref Vector3 v)
            {
        ...
        {
            public bool IsSubHole;
            public bool IsDirect;
            public List<Point> Hole;

            public HoleParam(List<Point> curHole, bool isSbHl = false, bool isDir = false) 
            {
                Hole = curHole;
                IsSubHole = isSbHl;
                IsDirect = isDir;
        ...
        public class SubPerimWrap
        {
            public List<Point> cSubPerim = null;
            public int cntHoles;
            public bool IsAdded = false;//Was added to subPerimeterPnts
            public int tIndx = -1;

            public SubPerimWrap(List<Point> linkSubPerim) 
            {
                cSubPerim = linkSubPerim;
            }
        }

        public class SubHoleWrap 
        {
            public int Index = -1;
            private List<int> outerSubPerimeters;
            private List<float> distances;
            private int curNearestSubPerimIndx = -1;
            public SubHoleWrap(int i) 
            {
                Index = i;
                outerSubPerimeters = new List<int>();
                distances = new List<float>();
            }   
            public void Add(int subPerimIndx, float dist)
            {
                int lastEl = distances.Count;
                outerSubPerimeters.Add(subPerimIndx);
                distances.Add(dist);
                bool flgNull = curNearestSubPerimIndx > -1;
                if (flgNull) 
                {
                    if (dist < distances[curNearestSubPerimIndx])
                    {
                        curNearestSubPerimIndx = lastEl;
                    }
                }
                else
                {
            ...
            public int Count ()
            {
                return outerSubPerimeters.Count;
            }  

            public int GetIndxSubPerim(int i)
            {
                return outerSubPerimeters[i];
            }
            public float DistanseAt(int i)
            {
                return distances[i];
            }
            public int NearestSubPerimIndx 
            {
                get { return outerSubPerimeters[curNearestSubPerimIndx]; }
            }
        }
		...
		public class Slice
        {
            ...
            public Slice(Point _p1, Point _p2, float _d1, float _d2, HoleAttribute holeAttr, int curHoleIndx, int j1, int j2)
            {
				...
				d[0] = _d1;
				d[1] = _d2;
				Lst = holeAttr.Lst[curHoleIndx].Hole;
            ...
            {
                Lst = new List<Point>(holeAttr.Lst[curHoleIndx].Hole);
		...
		public class Triangulation 
        {
            ...
            public static List<int> GeneralTriangulation(List<Vector2> v, int nPosit, Vector3 nrm, bool flgKnownDirect = false, bool flgCutter = false, int ii = 0) 
            {
                ...
                List<Vector2> copyV = new List<Vector2>();
                List<int> indxList = new List<int>();
                copyV.Capacity = v.Count;
                ...
                bool flg = true; //!flgCutter;
                if (!flgKnownDirect) 
                {
                    VectorCrosOut = VectorExtension.OrtCrosVect(copyV_t[ii], nLeft, copyV_t[ii + 2]);
                ...
                do 
                {
                    ...
                    if (!casher[index])
                    {
                        ...
                        float tmpOrt = VectorExtension.OrtCrosVect(copyV[before], curV, copyV[after]);//Outer (cross sign is like the normal by plane
                ...
                for (int i= 0; i< v.Count; i++) 
                {
                    ...
                    if (v[i] != A && v[i] != B && v[i] != C) 
                    {
                        ...
                        if (v23 * v43 < 0)
                        {
                            ...
                            if (proj1 >= dotACDbc && proj2 >= dotBCDac)
                            {//дб >=, потому что при 90 градусах будет =, но только 1 из Dot-ов сможет быть =0. Одновременно если, то 180 градусов будет мбду сторонами - не канает
...
//            public const float cFactor = 0.0001f;//for find cross Pnt
            ...
            public static bool IsCrossHorizontalRayWithDist(Vector2 p1, Vector2 p2, ref float r) 
            {
                Vector2 ray = Vector2.left;
                float f1 = Vector3.Cross(p1, ray).z;
                float f2 = Vector3.Cross(p2, ray).z;
                bool flg = (Mathf.Abs(Mathf.Sign(f1) + Mathf.Sign(f2)) < rFactor);
                if (flg) 
                {
                    float proj1 = Vector2.Dot(p1, ray);
                    float proj2 = Vector2.Dot(p2, ray);
                    bool positiveSumProject = proj1/p1.sqrMagnitude + proj2 / p2.sqrMagnitude > 0;
                    if (positiveSumProject) 
                    {
                        Vector2 rayOrtho = Vector2.up;
                        float koef = Vector2.Dot(p1, rayOrtho) / Vector2.Dot(p2, rayOrtho);
                        float sum = Mathf.Abs(proj2 - proj1);
                        r = proj1 + sum * koef / (koef + 1);
                    }
                    return positiveSumProject;
                }
                else 
                {
                    return false;
			...
			public static float OrtCrosVect(Vector2 v1, Vector2 v2, Vector2 v3)
            {
                Vector2 V01 = v1 - v2;
                Vector2 V02 = v3 - v2;
            ...
            {
                float f = 0f;//rFactor/10000f;
                Vector3 delt = v2.normalized - v1.normalized;

                //Если отношения координат равны
                if (delt.x < f && delt.x > -f && delt.y < f && delt.y > -f && delt.z < f && delt.z > -f) 
                {
                    return true;
                }
                return false;
